...
#### AHORA CONSIDERAMOS QUE EL CAMPO DE DIFRACCIÓN ES LA ENTRADA A LA RETROPROPAGACIÓN

campo_detector = campo_difraccion

fase_cuadratica_salida = (np.exp(-1j*k*z) * (1j*long_de_onda*z)) * np.exp(-1j*(k/(2*z)) * (X**2 + Y**2))

campo_difraccion = campo_detector * fase_cuadratica_salida
campo_sin_FFT = np.fft.fftshift(np.fft.ifftshift(np.fft.ifft2(campo_difraccion)))
...
im = ax[1].imshow(np.abs(campo_detector)**2, extent=extent, cmap="gray")
